...
if not all(ALL_API):
  st.sidebar.error("MUST PAS ALL API_KEYS")
...
def search_latest_info(query):
  """This function helps to give latest search using tavily based on
  given user query realted research or contents"""
...
def agent_prompt(query):
  ...
  based on given query by user, it give detailed professional prompt to return
  the output"""

  prompt = f"""Give detailed highly professional prompt for below given prompt.

  you are a professional ppt designer, based on user given query,
  your task is to professional HTML output prompt with no markdowns.
...
def run_agent(leader_agent, query):
  ...
  first to promptify user prompt, than call image tool, or latest search if required,
  give slide dynamic, ui ux,
  with creative design, keep help to function to generate image and embed or use
  direct url based on given topic, Generate image as no. of slides
  and imbed that in same html ppt
  and using file handling embed this in output html, use java script function
  to generate image using async func and threading and
...
if all (ALL_API):
  ...
  st.info("Pass- All Aip-Keys and rerun")
